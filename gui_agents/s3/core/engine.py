...
class LMMEngineLlhama(LMMEngine):
    """Engine adapter for Llhama-backed models.

    Behavior:
    - If `base_url` is provided, it assumes an OpenAI-compatible HTTP endpoint
      (chat completions) and forwards requests via the existing OpenAI client.
    - Otherwise, supports a simple local mode that runs a subprocess command
      (`local_cmd`) with the prompt on stdin and captures stdout as the response.

    This is intentionally lightweight: adapt to your Llhama deployment by
    providing either a compatible HTTP server or a small wrapper script
    exposed via `local_cmd`.
    """

    def __init__(
        self, base_url=None, api_key=None, model=None, local=False, local_cmd=None, **kwargs
    ):
        assert model is not None, "model must be provided"
        self.model = model
        self.base_url = base_url
    ...
    def generate(self, messages, temperature=0.0, max_new_tokens=None, **kwargs):
        # If an HTTP endpoint is available, call it directly (no API key required for local Ollama)
        if self.base_url:
            try:
                import json
                ...
                # Navigate response similar to OpenAI chat completions
                return parsed["choices"][0]["message"]["content"]
            except Exception as e:
                raise RuntimeError(f"Failed to call Llhama HTTP endpoint: {e}")

        # Local mode: convert messages to a single prompt and call subprocess
        if self.local:
            if not self.local_cmd:
                raise ValueError("local_cmd must be provided when using local Llhama mode")

            prompt_parts = []
            for m in messages:
                role = m.get("role", "user")
                # content is a list of dicts with type/text
                parts = []
            ...
            if proc.returncode != 0:
                raise RuntimeError(f"Llhama subprocess failed: {err}")
            return out.strip()

        raise ValueError("Either provide base_url for HTTP mode or set local=True with local_cmd")
